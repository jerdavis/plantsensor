...
ChirpDevice::ChirpDevice(i2c::I2CComponent *i2c_bus, uint8_t address)
...
void ChirpDevice::reset() {
  ...
  if (!this->i2c_bus_->write(this->address_, &reset_cmd, 1)) {
...
optional<uint16_t> ChirpDevice::read_register_16bit_(uint8_t reg) {
  ...
  if (!this->i2c_bus_->write(this->address_, &reg, 1)) {
  ...
  // Read 2 bytes
  uint8_t data[2];
  if (!this->i2c_bus_->read(this->address_, data, 2)) {
...
bool ChirpDevice::write_register_(uint8_t reg, uint8_t value) {
  ...
  if (!this->i2c_bus_->write(this->address_, data, 2)) {
